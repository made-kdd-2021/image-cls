...
def main(config):
    ...
    model_trainer: PneumoniaClsTrainer = utils.instantiate(train_config.model_trainer,
                                                           loss_module=None,
                                                           model=model,
                                                           _recursive_=False)

    trainer: Trainer = utils.instantiate(train_config.trainer)
    ...
    result = trainer.predict(model=model_trainer, dataloaders=loader,
                             ckpt_path=config.checkpoint_path)
    save_metrics(metric_dir, result)
