...
class PneumoniaClsTrainer(LightningModule):
    ...
    def _update_metrics(self, predicted_logits: Tensor, true_labels: Tensor):
        ...
        self._confusion_metric.update(predicted_logits, true_labels)
    ...
    def _compute_loss(self, image_batch_info: InputType):
        labels = image_batch_info["labels"]
        images = image_batch_info["images"]
    ...
    def training_step(self, image_batch_info: InputType):
    ...
    def _compute_metrics(self):
        conf_matrix = self._confusion_metric.compute()
        auc_roc = self._auc_roc.compute()
    ...
    def _log_metrics(self, prefix_stage: str):
        ...
        if self.logger is not None:
            ...
            self.logger.experiment.add_figure(
                f"{prefix_stage}/ROC curve", fig, global_step=self.current_epoch)

        self.log(f"{prefix_stage}/AUC ROC", auc_roc)

    def training_epoch_end(self, outputs) -> None:
        super().training_epoch_end(outputs)
        self._log_metrics(self._train_prefix)

    def validation_step(self, image_batch_info: InputType):
        predicted_logits, labels, loss_value = self._compute_loss(image_batch_info)
    ...
    def test_step(self, image_batch_info: InputType):
    ...
    def test_epoch_end(self, outputs) -> None:
        ...
        conf_matrix, auc_roc, (fpr, tpr, _) = self._compute_metrics()
        self.log_dict({"conf_matrix": conf_matrix.cpu().numpy(), "auc_roc": auc_roc.item()})

    def configure_optimizers(self):
        optimizer = instantiate(self._model.parameters(), **self._optimizer_config)

        if self._scheduler_config is not None:
            scheduler = instantiate(optimizer, **self._scheduler_config)
